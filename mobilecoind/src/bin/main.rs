...
fn main() {
    ...
    let mr_signer_measurement = mc_consensus_enclave_measurement::mr_signer_measurement(None);

    log::debug!(logger, "MRSIGNER measurement: {:?}", mr_signer_measurement);

    // Create peer manager.
    let peer_manager = config
        .peers_config
        .create_peer_manager(mr_signer_measurement, &logger);
