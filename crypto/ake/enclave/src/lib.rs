...
use mc_attest_verifier_config::{TrustedMeasurement, TrustedMrEnclaveMeasurement};
...
impl<EI: EnclaveIdentity> AkeEnclaveState<EI> {
    ...
    /// Get measurements which ensures MRENCLAVE matches
    fn trusted_measurement(&self) -> Result<TrustedMeasurement> {
        let report_body = Report::new(None, None)?.body();

        let mr_enclave = report_body.mr_enclave().into();
        ...
        Ok(TrustedMrEnclaveMeasurement::new(
            &mr_enclave,
            [] as [&str; 0],
    ...
    fn get_verifier(&self) -> Result<Verifier> {
        ...
        verifier
            .measurements([&self.trusted_measurement()?])
            .debug(DEBUG_ENCLAVE);
    ...
    pub fn peer_connect(
        ...
        let measurements = [self.trusted_measurement()?];
